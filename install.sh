...
install_binary() {
    ...
    if command -v curl >/dev/null 2>&1; then
        HTTP_CODE=$(curl -L -w "%{http_code}" -o "${ARCHIVE_PATH}" "$DOWNLOAD_URL" 2>/dev/null)
        if [ "$HTTP_CODE" != "200" ]; then
            log_error "Failed to download release (HTTP $HTTP_CODE)"
            log_info "This may mean the release is still being built."
            log_info "Try again in a few minutes or check: https://github.com/${REPO}/releases"
            exit 1
        fi
    else
    ...
    if [ ! -s "${ARCHIVE_PATH}" ] || file "${ARCHIVE_PATH}" | grep -q "HTML\|text"; then
        log_error "Downloaded file is not a valid archive"
        log_info "The release may still be building. Try again in a few minutes."
    ...
    if [ ! -f "$BINARY_PATH" ]; then
        # Look for binary with version suffix or platform suffix (excluding archives)
        VERSIONED_BINARY=$(find "$TMP_DIR" -name "${BINARY_NAME}*" -type f ! -name "*.tar.gz" ! -name "*.zip" ! -name "*.md" ! -name "LICENSE*" ! -name "*.txt" | head -1)
        if [ -n "$VERSIONED_BINARY" ]; then
            BINARY_PATH="$VERSIONED_BINARY"
        else
            # Last resort: look for any executable file
            BINARY_PATH=$(find "$TMP_DIR" -type f -perm +111 ! -name "*.md" ! -name "LICENSE*" ! -name "*.txt" | head -1)
